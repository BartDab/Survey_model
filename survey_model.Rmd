...
```{r}
levels(df0$Wielkosc_miasta)<-c('poniżej 5 tysięcy','5 - 20 tysięcy', '20 - 100 tysięcy', '100 - 200 tysięcy', '200 - 500 tysięcy', 'powyżej 500 tysięcy')
data2<-df0%>%
...
g4<-ggplot(data2, aes(x=data2$Wielkosc_miasta,y=data2$n,fill=data2$Wielkosc_miasta))+geom_bar(stat='identity')+ theme(legend.position = "none")+
  geom_text(aes(label = n), position =position_dodge(width=0.9), vjust=-0.25)+
  xlab('')+ylab('')+ ggtitle('Wielkość miasta:')+theme(plot.title = element_text(hjust = 0.5))+scale_color_viridis(option = "D")+ylim(0,155)
g4
```
...
```{r}
levels(df0$Rok)<-c('Ukończyłem/ukończyłam studia','V','IV','III','II','I')
data2<-df0%>%
  dplyr::group_by(Rok)%>%
  dplyr::count()
g6<-ggplot(data2, aes(x=data2$Rok,y=data2$n,fill=data2$Rok))+geom_bar(stat='identity')+ theme(legend.position = "none")+
  geom_text(aes(label = n), position =position_dodge(width=0.9), vjust=-0.25)+
  xlab('')+ylab('')+ ggtitle('Rok studiów')+theme(plot.title = element_text(hjust = 0.5))+scale_color_viridis(option = "B")+coord_flip()
g6
levels(df0$Rok)<-c('I','II','III','IV','V','Ukończyłem/ukończyłam studia')
```
