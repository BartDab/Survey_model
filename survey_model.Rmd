...
```{r}
model_rf<-randomForest::randomForest(Praca~.,data=df, importance=TRUE)
print(model_rf)
importance(model_rf)
varImpPlot(model_rf)
```
...
```{r}
model2<-randomForest::randomForest(Praca~.,data=df4,importnace=TRUE)
print(model2)
importance(model2)
varImpPlot(model2)
table(df4$Praca)
...
```{r}
model3<-randomForest::randomForest(Praca~.,data=df5,importance=TRUE)
print(model3)
importance(model3)
varImpPlot(model3)
```
...
```{r}
set.seed(42)
model4<-randomForest(Praca~.,data=df6,ntree=1000,mtry=5,importance=TRUE,sampsize=c(155,155))
print(model4)
...
```{r}
table(df01$Praca)
model5<-randomForest(Praca~.,data=df01,ntree=500,mtry=6,importance=TRUE,sampsize=c(150,150))
print(model5)
importance(model5,class=TRUE)
plot(model5) #zielony - pracuję, czarny - średni, czerwony - jeszcze nie
varImpPlot(model5)
#str(model5)
...
```{r}
(err<-(model5$confusion[2]+model5$confusion[3])/400)
(acc<-(model5$confusion[1]+model5$confusion[4])/400)
(sens<-model5$confusion[1]/(model5$confusion[1]+model5$confusion[3]))
(spec<-model5$confusion[4]/(model5$confusion[4]+model5$confusion[2]))
(gmean<-sqrt(sens*spec))
(recall<-model5$confusion[1]/(model5$confusion[1]+model5$confusion[2]))
(prec<-model5$confusion[1]/(model5$confusion[1]+model5$confusion[3]))
```

###PRÓBA ELIMINACJI NIEPOTRZEBNYCH ZMIENNYCH#
...
```{r}
table(df02$Praca)
model6<-randomForest(Praca~.,data=df02,ntree=500,mtry=6,importance=TRUE,sampsize=c(150,150))
print(model6)
importance(model6,class=TRUE)
plot(model6) #zielony - pracuję, czarny - średni, czerwony - jeszcze nie
varImpPlot(model6)
#str(model5)
